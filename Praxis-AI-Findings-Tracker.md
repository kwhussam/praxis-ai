...
| F-002 | 1 | Doku-Konsistenz | Architektur-Dokumentation nicht deckungsgleich mit Code, u.a. bei Auth-Speicherung (Code nutzt SecureStore) | docs/ARCHITECTURE.md vs. lib/store/secureAuthStorage.ts | Mittel | Offen |
| F-003 | 1 | Dead Code / Backend-Redundanz | Supabase Edge Functions (ai-report, external-check) werden vom App-Code nicht mehr aufgerufen; produktiver Pfad läuft ausschließlich über Hono Worker. Funktionen existieren aber noch im Repo. | supabase/functions/, lib/api/client.ts, lib/security/external.ts, lib/ai/report.ts | Mittel | Offen |
| F-004 | 1 | Dead Code | Legacy-Kompatibilitätsrouten im Hono Worker (/api/external-check, /security/external, /ai/report) ohne erkennbaren aktiven Aufrufer im App-Code | workers/hono/src/index.ts | Niedrig-Mittel | Offen |
...
| F-012 | 2 | Sicherheit / Stub-Gefahr | Supabase Edge Function `external-check` liefert immer einen hartcodierten DMARC-Fake-Finding (`scoreImpact: -9`) — aktuell ungenutzt, aber gefährlich falls versehentlich angebunden/deployt (Bezug zu F-003) | supabase/functions/external-check/index.ts | Hoch | Offen |
...
| F-030 | 3 | Dead Code / Angriffsfläche | Legacy-Endpunkte `/api/external-check`, `/security/external`, `/ai/report` überlappen funktional mit den neuen Endpunkten und werden vom Frontend nicht genutzt (bestätigt F-004) | workers/hono/src/index.ts:420 | Mittel | Offen |
...
| F-035 | 3 | Bestätigung von F-003/F-012 | Supabase Edge Function `external-check` bleibt als Stub mit hartcodiertem Fake-Finding bestehen, parallel zum produktiven Worker | supabase/functions/external-check/index.ts:21 | Bezug F-003/F-012 | Offen |
...
| F-074 | 6 | Architektur / Code-Duplikation | Doppelte AI-Report-Logik (Anthropic-Aufruf + Parsing) parallel in Supabase Edge Function und Hono Worker (Bezug F-003/F-012/F-035) | supabase/functions/ai-report/index.ts:40,193, workers/hono/src/index.ts:1203,607 | Hoch | Offen |
...
| F-076 | 6 | Dead Code (bestätigt F-008/F-057) | `DomainCheck`-Komponente exportiert, aber nirgends importiert/verwendet; `runExternalCheck` als Service getestet, aber Check-Flow ruft ihn nie auf | components/modules/DomainCheck.tsx:16, lib/security/external.ts:158, app/(tabs)/check/index.tsx:28 | Mittel | Offen |
