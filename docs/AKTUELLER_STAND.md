...
Stand: 2026-09-19 (SP3-03 Assessment-Snapshot v1 lokal implementiert)
...
- **SP3-02 einschließlich G-01 gemergt:** PR
  ...
  `quality`, `rls-pgtap`, Dependency Review, Dependency/SBOM und CodeQL SAST sind grün.
...
autoritative Snapshotvertrag additiv implementiert:
...
1. **SP3-03 committen, pushen und per PR prüfen.** Erforderliche GitHub-Gates sind `quality`,
   `rls-pgtap`, Dependency Review, Dependency/SBOM und CodeQL SAST. Die Migration darf erst nach
   grüner CI gemergt werden.
2. **Nach Merge SP3-04 – SafeScan-Policy und Scan-Authorization-Schema beginnen.** Das baut auf
   Snapshot-Komponenten und dem bestehenden Collection-Vertrag auf, aktiviert aber noch keinen
   produktiven aktiven Scan.
3. **Engine-Cutover bleibt separat:** Erst nach ADR-001- sowie D-05/D-06/D-07-Freigabe darf ein
   produktiver v2-verschlüsselter Snapshot erzeugt und an neue Reportmanifeste gebunden werden.
4. **Dringendste inhaltliche Klärungen**, unabhängig von der Phasenfolge:
...
5. Die produktionsnahe Auth-/Provider-Wiederherstellung bleibt an D-02 gebunden; Phase C der
   Schlüsselrotation bleibt durch D-05/D-06/D-07 blockiert.
6. Parallel als separaten Runtime-Nachweis den seriellen SDK-57-Maestro-Lauf wiederholen:
...
7. Android-Smoke und physische iOS-/Android-Gerätematrix bleiben separate Produktions-Gates.
