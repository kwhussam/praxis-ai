...
| B-M-03 | P0-03, P1-01 | SP2-04 in verification; technisches Review ohne P1/P2 bestanden, Native-Smokes und Datenschutzfreigaben offen |
...
Stand: **2026-08-13**, lokaler Arbeitsstand; noch kein Produktionsrelease.
...
| SP2-01A | verification | Versioniertes Inventarrepository mit praxisgebundener AES-256-GCM-Verschlüsselung, SecureStore-DEK, verschlüsseltem SQLite-Snapshot, CAS-Revisionen, bestätigtem `saving`→`ready`, Hydrierungs-/Praxiswechsel-Raceschutz, Herkunftsfeldern, hartem Seed-Syncblock und sichtbarem Volatile-/Fehlerstatus implementiert; nativer iOS-18.6-Simulatorlauf für Neustart, Logout/Login und Zwei-Praxis-Isolation bestanden; SQLite extern klartextfrei geprüft | physische Android-/iOS-Device-Smokes für Keystore/Keychain und SQLite; Android aktuell ohne verfügbares AVD/Gerät; SP2-01B Delta-/Cloudsync bleibt bis ADR-001/V0-Freigabe gesperrt |
| SP2-03 | released | Getrennte Praxis-/Techniksicht; Praxisansicht mit autoritativer Ampel, begrenzter Klartextaussage, drei Maßnahmen und sichtbarer Evidenzabdeckung; Technikansicht mit Teilwert, Coverage, Confidence, Freshness, Regel-Evidenz und ausschließlich vergleichbarer Fragebogenhistorie; gemeinsame `deriveScoreReportPosture`-Ableitung verhindert Schwellen-/Freshness-Drift zwischen Dashboard und Praxistext; WLAN-/Monitoring-Coverage im Dashboardvertrag; Claims „sicher“, „echte Prüfdaten“ und „Live“ im betroffenen Pfad entfernt; Tarifkarte bleibt außerhalb der Risikohierarchie; A11y-, Semantik-Snapshot-, Selector- und Worker-Contract-Tests implementiert; nativer iOS-18.6-Smoke auf einem iPhone-SE-Viewport bei Standardgröße und `accessibility-extra-large` bestanden; responsive Umbrüche für Risikokarte, letzte Prüfungen, Ansichtsumschalter und Technikmetriken verifiziert | keine SP2-03-Arbeit offen; physische Gerätematrix bleibt separates Gate in SP2-06/P0-09 |
| SP2-04 | verification | Unveränderliches, versioniertes und AES-256-GCM-verschlüsseltes Assessment-Manifest; explizit definierte kanonische JSON-/Zahlenregeln; transaktionale/idempotente Manifest+Report-RPC; tenantgebundener serverseitiger Mehrseiten-PDF-Pfad mit Integritätsprüfung und byte-identischer Reproduktion; Decrypt-Fehler liefern fail-closed 409; lokaler HTML-Renderer entfernt; App öffnet Serverbytes über den nativen Teilen-/Öffnen-Dialog, hält sie nur tenantgetrennt in `cacheDirectory` und löscht die Klartextdatei nach Dialogende/Fehler sowie bei Logout/Praxiswechsel; Reporthistorie wird nach Neustart geladen; Altberichte ohne Manifest bleiben lesbar und scheitern beim kanonischen Export geschlossen; Datenschutzexport und Lösch-RPC erweitert; technisches Review ohne P1/P2 bestanden; 19 neue pgTAP-Beweise für RLS, Grants, Cross-Tenant-FK, Atomicity und Idempotenz; GitHub-CI-Run `31600094527` mit `quality`, Gitleaks und `rls-pgtap` vollständig grün; echter nativer iOS-PDF-Dialog mit Server-PDF und Vorschau nachgewiesen | vollständiger PDF-Export-/Öffnen-/Cleanup-Smoke auf iOS und Android (Android ohne lokales AVD/Gerät); Datenschutz-Sign-off/ADR-001 sowie externe D1-Rechtsprüfung vor `released` |
| SP2-05 | verification | Append-only Consent Registry v1 für Provider- und getrennte HIBP-Freigabe; serverautoritatives Scope-/Textversions-/Ablaufmodell mit Actor- und Vorgängerevidenz; Manager-Write-/Viewer-Read-Endpunkte; service-role-only Entscheidungs-RPCs; manuelle und geplante Läufe prüfen fail-closed vor Quote und Provider; Monitoring-UI zeigt Empfänger, Datenarten, Status, Ablauf und unmittelbaren Widerruf; Datenschutzexport erweitert; frischer Datenbankreset, 17 neue pgTAP-Beweise und insgesamt 242 pgTAP-Tests grün; GitHub-CI-Run `31616201072` mit `quality`, Secret Scan und `rls-pgtap` grün; unabhängiges Review ohne P1/P2/P3, beide P4-Driftrisiken geschlossen | Datenschutz-/Fachrechtsfreigabe für Wortlaut, Rechtsgrundlage, Drittanbieterinformationen, Aufbewahrung und Widerrufsfolgen; iOS-/Android-Smoke für Persistenz und Folgescan |
| SP2-06 | verification | Native Permission-/Buildvertrag aus `app.json` und eingecheckten Config-Plugins; iOS Wi-Fi-Information-Entitlement, enge Purpose Strings und Entfernung ungenutzter Bonjour-Claims; Swift-/ObjC-Network-Probe aus Clean-Prebuild reproduzierbar; Android API 35 mit explizitem Fine-Location-/Nearby-Wi-Fi-Vertrag; Storage-/Overlay-Rechte entfernt, Backup und Geräteübertragung vollständig ausgeschlossen, Klartextverkehr aus; Debug-Keystore aus Release-Buildtyp entfernt; CI erzeugt beide Native-Projekte, kompiliert ein unsigniertes Android-Release und prüft alle Merge-Manifeste sämtlicher Release-Flavors; iOS-mDNS ist explizit `unsupported` und damit keine dauerhaft unerfüllbare Coverage-Quelle; Clean-Prebuild sowie nativer iOS-18.6-Simulator-Build, Installation und Start bestanden; GitHub-CI-Run `31705322225` einschließlich Release-Lint, Android-`assembleRelease`, Merge-Manifest, Secret-Scan, Gesamtverifikation und `rls-pgtap` vollständig grün; unabhängige P4-Hinweise zu Flavor-Auswahl und iOS-mDNS-Coverage geschlossen | physische iOS-/Android-Smokes laut `SP2_06_NATIVE_RELEASE_RUNBOOK.md`, Android-Minimum-API-23 und abschließender Production-/Store-Signing-Nachweis |
| SP3-01 | verification | Separate Secure-SDLC-Pipeline mit registry-aktuellem, GHSA-genauem Dependency-Gate, maximal 31 Tage gültigen Build-Toolchain-Ausnahmen, PR-Dependency-Review, CodeQL `security-extended` und eigenem SARIF-Gate; validierte CycloneDX-SBOM als CI-Evidenz; alle Actions per Commit-SHA gepinnt und Dependabot-Updates eingerichtet; Release-Temp-Pfade werden erst im Runner aufgelöst; geschützte Android-/iOS-Release-Jobs mit fail-closed Secretvollständigkeit, freigegebenem Zertifikatsfingerabdruck, Tag-/Main-Bindung, produktiver Endpoint-Konfiguration, Hashmanifest und GitHub/Sigstore-SBOM-Attestation; `xmldom` 0.9.11 bleibt mit reproduzierbarem Expo-Plist-Patch prebuild-kompatibel; CI `31794686220` und Secure SDLC `31794686209` vollständig grün; Patch-SLAs und vertrauliche Vulnerability Disclosure dokumentiert | GitHub Branch-/Tag-/Environment-Regeln; echte signierte AAB-/IPA-Releases und unabhängige Attestation-/Store-Identitätsprüfung; koordinierter Expo-/React-Native-Upgrade vor Ablauf am 13.09.2026; benannte Triage-Owner |
| SP3-01B | merged / production-gate-open | Expo SDK 52 bis 57 kontrolliert migriert; Zielstand SDK 57 / RN 0.86.3 / React 19.2.3, Hermes-v1-Rückstand geschlossen, Expo Doctor 21/21, Dependency-Allowlist leer; PR #51 als `af8d579` gemergt, Post-Merge-CI `35102657829` und Secure SDLC `35102657678` grün | vollständiger 15/15-Simulator-Wiederholungslauf nach den letzten Harness-Fixes; physische iOS-/Android-Gerätematrix sowie echter Signing-/Store-Nachweis bleiben Produktions-Gates |
| SP3-02 | Phase A `audit_completed`; Phase B `technical_complete_8_of_8`; Release `blocked_by_owner_decisions` | Basis über PR #59 als `715f3ab` gemergt; **Phase A vollständig durchgeführt**: `docs/SP3_02_RECOVERY_INVENTORY.md` inventarisiert am tatsächlichen Code zehn Bestandsgruppen und 24 priorisierte Lücken; `docs/SP3_02_DECISION_LOG.md` führt zehn offene Entscheidungen D-01 bis D-10. **Phase B ist reproduzierbar implementiert und lokal gemessen:** synthetischer Zwei-Mandanten-Dump, isolierte Restore-Datenbank und Metadaten-only-Evidenz. Nach der freigegebenen G-01-Migration löschen RPC und Restore alle sechs D2-Sammlungen mandantenfest; vollständiger Vorabexport, Legal Retention und Wiederholungslauf sind geprüft. Referenzlauf auf Commit `5705b69` 8/8 mit 266/266 pgTAP-Assertions, identischen RLS-/Policy-/Grant-Snapshots, identischen kanonischen Report-/PDF-Hashes und fail-closed Schlüsselprüfung. Lokale Messung: 291 ms Backup, 443 ms Restore; kein Produktionsclaim | G-01-Folge-PR mit grüner CI/Secure SDLC mergen; null von zehn Owner-Entscheidungen sind `decided`, daher bleiben Produktionsübertragbarkeit und Freigabe gesperrt; Phase C benötigt D-05/D-06/D-07, Phase D D-08/D-09/D-10; RPO/RTO bleiben unbestimmt bis produktionsrepräsentativer Messung **und** Freigabe. Danach beginnt SP3-03 auf neuem Branch vom aktuellen `main` |
| SP3-03 | implementation / local verification green | ADR-002, strikter JSON-/Runtime-Vertrag und zwei Contract Fixtures; additive Tabellen für unveränderliche Snapshots, geordnete Komponenten und Scoreerklärungen; atomare/idempotente service-role-only RPC, `FORCE RLS`, entzogene Direktwrites und ausgeblendetes Ciphertextfeld; optionale echte Ed25519-Signatur statt falschem Hash-Claim; nullable tenantfeste Manifestbindung; Datenschutzexport und Praxislöschung erweitert. Frischer DB-Reset sowie 300/300 pgTAP-Assertions grün, davon 34 neue Snapshot-Prüfungen; Lint/TypeScript und 512 Jest-Tests grün; Recovery-Drill mit erhaltenem Praxis-A-Snapshot und gelöscht bleibendem Praxis-B-Snapshot 8/8 | Commit/Push, PR mit grüner CI und Secure SDLC, unabhängiges Security-/Privacy-Review. Produktive Snapshoterzeugung und Manifest-Cutover bleiben bis ADR-001 und D-05/D-06/D-07 gesperrt; danach SP3-04 |

Verifikation dieses Arbeitsstands: `npm run verify` ist grün (Lint, TypeScript, **424 bestandene Tests**, 6 bewusst übersprungen, 2 bestandene Semantik-Snapshots). Die erwarteten Warn-/Fehlerlogs stammen aus simulierten Provider- und Auditfehlerfällen der Tests. Der SP2-06-Clean-Prebuild aus einem Checkout ohne Native-Verzeichnisse sowie ein unmittelbar wiederholter idempotenter Prebuild bestanden; das generierte iOS-Projekt bindet beide Probe-Quellen ein und das Android-Release besitzt weder Debug-Signing noch Backup-/Klartextfreigaben. Der native iOS-18.6-Build wurde installiert und gestartet; Xcodes simuliertes Entitlement-Artefakt enthält Wi-Fi Information. Der Android-Compile läuft nach Push im neuen Java-17-CI-Gate, da lokal weder JDK noch AVD/Gerät installiert sind. `supabase db test --local` bestand vollständig mit **242 pgTAP-Tests**; neben der SP2-04-Suite prüft die neue SP2-05-Suite 17 Grant-, RLS-, Ablauf-, Versions-, Scheduler-, Append-only- und Vorgängerketteneigenschaften ohne optionale Jest-Umgebungsvariablen. Ein frischer Datenbank-Reset wandte alle Migrationen einschließlich SP2-05 an; der Datenbank-Linter meldete keine neue SP2-05-Warnung. GitHub-CI-Run `31616201072` für Commit `b253d76` bestand vollständig: `quality` einschließlich Secret Scan und Gesamtverifikation sowie `rls-pgtap` sind grün. Das anschließende unabhängige Review fand keine P1-/P2-/P3-Befunde; beide P4-Driftrisiken sind mit gemeinsamer RPC-Autorität und festem Praxiszugriff geschlossen. Zusätzlich wurden die iOS- und Android-Metro-Exporte mit den neuen Crypto-/SQLite-Modulen erfolgreich erzeugt. Der SP2-01A-Maestro-Flow bestand im nativen iOS-18.6-Simulator; SQLite und Keychain-Metadaten wurden danach unabhängig von der App auf Mandantentrennung und Klartextfreiheit geprüft. Der eigenständige SP2-03-Flow `14-dashboard-responsive` bestand auf iOS 18.6 im iPhone-SE-Viewport sowohl bei Standardschrift als auch mit `accessibility-extra-large`; dabei gefundene Überläufe in Ergebniszeilen, Risikoüberschrift, Tab-Beschriftungen, Umschalter und Technikmetriken wurden behoben. Für SP2-04 wurde das echte kanonische Server-PDF samt Vorschau im nativen iOS-Dialog nachgewiesen; der vollständige automatisierte Cleanup-Smoke bleibt wegen fehlendem Android-Gerät offen.

Nachtrag zum SP2-06-Nachweis: Der oben noch als laufend bezeichnete Android-Compile ist abgeschlossen.
GitHub-CI-Run `31705322225` für Commit `8af2d52` bestand vollständig: Das Java-17-/NDK-Gate
...
Nachtrag zum SP3-01-Arbeitsstand: Die vollständige lokale Verifikation ist mit **439 bestandenen
...
Der Status bleibt bis zum geschützten Produktions-/Signing-Nachweis `verification`.

GitHub bestätigte den technischen Stand für Commit `a26b8cf`: CI-Lauf `31794686220` bestand
`quality` einschließlich Clean-Prebuild, Android-Release-Compile und Merge-Manifest sowie
`rls-pgtap`; Secure-SDLC-Lauf `31794686209` bestand Dependency-/SBOM-Gate und CodeQL. SP3-01 bleibt
bis zur geschützten Produktions-/Signing-Konfiguration und zum echten signierten Plattformnachweis
in `verification`.
...
Nicht als erledigt markiert bleiben die vollständige Claim-Inventur, die physische Android-/iOS-Gerätematrix einschließlich des vollständigen SP2-04-PDF-Cleanup-Smokes und die freizugebende Datenmigration. Das technische SP2-04-Review ist ohne P1/P2 bestanden; vor `released` fehlen weiterhin Datenschutz-Sign-off/ADR-001 und die externe D1-Rechtsprüfung. Der vorbestehende Lösch-RPC-Umfang für Inventar-/Monitoring-Target-Tabellen bleibt ausdrücklich ADR-001-Blocker vor M5.
...
### 22.3 SP3-02 – Ergebnis der Phase-A-Istaufnahme

Die Istaufnahme vom 2026-09-16 (`docs/SP3_02_RECOVERY_INVENTORY.md`) ist eine Prüfung am tatsächlichen Code, **kein getesteter Restore**. Sie hat weder eine Datenbank wiederhergestellt noch ein Backup gelesen oder erzeugt, noch einen Schlüssel erzeugt, rotiert, widerrufen oder angezeigt; Cloudflare-, Supabase-, GitHub-Environment-, KMS- und Signing-Konfigurationen blieben unverändert. Die Lieferung besteht ausschließlich aus Dokumentation.

Die vier gewichtigsten Befunde:

- **`DATA_ENCRYPTION_KEY` ist der kritischste Einzelpunkt des Produkts.** `workers/hono/src/index.ts:2416-2451` belegt einen einzigen globalen AES-256-GCM-Schlüssel für alle Mandanten, ohne Keyversion im Envelope, ohne AAD, ohne Wrapping und ohne zweite Version. Er entscheidet über die Lesbarkeit von `security_checks.encrypted_payload`, `reports.encrypted_content`, `assessment_manifests.encrypted_snapshot` und `monitoring_snapshots.encrypted_checks`. Sein Verlust macht die verschlüsselten Vollberichte und Snapshots aller Mandanten unlesbar und verhindert kanonische PDF-Reproduktionen; Datenbankzeilen und Klartextzusammenfassungen bleiben vorhanden. Seine Kompromittierung trifft alle verschlüsselten Mandantendaten gleichzeitig. Eine Rotation ist am heutigen Stand **technisch nicht durchführbar**, weil ohne Keyversion kein Mischzustand lesbar wäre. Verwahrung, Escrow und Wiederbeschaffungspfad sind repositoryweit `unknown`. Dies ist der Iststand zu dem in ADR-001 Abschnitt 6.1 beschriebenen, nicht implementierten Zielzustand.
- **Die längste Wiederherstellungszeit kann außerhalb der Datenbank liegen.** Falls ein Restore einen neuen Supabase-Host benötigt und kein stabiler Proxy-/Alias-/Failoverpfad existiert, erzwingt die in ausgelieferten Binaries gebundene URL einen Store-Release mit Reviewdauer. Die RPO/RTO-Entscheidung muss deshalb die Szenarien „bestehender Endpoint" und „Endpointwechsel" getrennt behandeln. Parallel ist der Release-Signing-Recoverypfad selbst unbelegt – insbesondere ist nicht dokumentiert, ob Google Play App Signing aktiv ist, wovon abhängt, ob ein Keyverlust überhaupt behebbar ist.
- **Der bestätigte ADR-001-Löschblocker wird durch die Restorebetrachtung verschärft.** `complete_privacy_deletion` erfasst `inventory_items`, `inventory_known_devices`, `inventory_access_points`, `router_wifi_configurations`, `router_firewall_rules` und `monitoring_targets` weiterhin nicht. Zusätzlich neu: Ein Restore auf einen Zeitpunkt vor einer vollzogenen Löschung stellt gelöschte D2-Daten wieder her, während `deletion_requests` sie als `completed` führt. Ein Abgleich, der diesen Widerspruch erkennt, existiert nicht.
- **Der AVV schreibt `data_region = 'EU / Frankfurt'` fest ein** – in `supabase/migrations/20260624150000_initial_schema.sql` (`create_default_avv`) und `workers/hono/src/index.ts:2966` –, ohne dass das Repository einen technischen Beleg für diese Zusage enthält. Das ist eine vertragliche Aussage gegenüber jeder Praxis; die Klärung ist Entscheidung D-03 und bewusst keine Codeänderung dieser Stufe.

Positiv belegt: 34 Anwendungstabellen mit `enable` **und** `force row level security`, aktuell 266 pgTAP-Assertions, append-only Consent-Registry mit Vorgängerkette und entzogenen `update`/`delete`/`truncate`-Rechten, transaktionale Manifest-/Reporterzeugung mit Hashkette und fail-closed Providerstatus (`not_configured` statt „kein Risiko"). Android schließt Appdaten vollständig aus Cloudbackup und Gerätetransfer aus; auf iOS sind SecureStore-Schlüssel mit `WHEN_UNLOCKED_THIS_DEVICE_ONLY` nicht übertragbar, während der Backupstatus des verschlüsselten SQLite-Ciphertexts repositoryweit `unknown` bleibt. Der lokale Kryptovertrag (`lib/inventory/localInventoryCrypto.ts`) ist mit echter AAD und Versionsfeldern strenger als der Cloudpfad.

Nachtrag vom 17. September 2026: Die ausdrücklich freigegebene G-01-Nachbesserung erweitert
`complete_privacy_deletion` atomar um alle sechs D2-Sammlungen und den mandantengebundenen
Vorabexport. Eine neue Suite prüft RPC-Grants, Zielmandant, Fremdmandant, Legal Retention und
wiederholte Ausführung. Insgesamt 266 pgTAP-Assertions sowie der Restore-Drill P-01 bis P-08 sind
grün; Phase B ist damit technisch abgeschlossen.

SP3-02 bleibt dennoch `blocked_by_owner_decisions`. Null von zehn Entscheidungen in
`docs/SP3_02_DECISION_LOG.md` sind `decided`; es wurde bewusst kein Zielwert, Provider, Region,
Frist oder Owner erfunden. RPO und RTO gelten nicht als erfüllt oder verbindlich, solange kein
produktionsrepräsentativer Restore und keine Owner-Freigabe existieren.
