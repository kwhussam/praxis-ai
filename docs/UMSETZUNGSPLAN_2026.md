...
| SP3-02 | Phase A `audit_completed`; Phase B lokal `ready`; Release `blocked_by_owner_decisions` | isolierter Branch `codex/sp3-02-recovery-tabletop`; **Phase A vollständig durchgeführt**: `docs/SP3_02_RECOVERY_INVENTORY.md` inventarisiert am tatsächlichen Code zehn Bestandsgruppen (Supabase/Postgres mit 34 RLS-erzwungenen Tabellen einschließlich `partner_plan_pricing`, Auth, nicht genutztes Storage, Migrationen/RLS/Grants/54 öffentliche Funktionen, Worker und sechs Secretklassen, Report-/Manifestverschlüsselung, sechs lokale Geräteschlüssel-/Persistenzpfade, Release-Signing, CI-/SBOM-/Attestationsartefakte, neun externe Provider) mit Datenklasse D0–D3, Plattform, Backupautorität, Schlüsselabhängigkeit, Wiederherstellungsstufe R0–R8, Retention-/Löschbezug und Nachweisstatus `measured`/`configured`/`documented`/`unknown`; daraus 24 priorisierte Lücken (7×P1, 8×P2, 6×P3, 3×P4) und der Entwurf des synthetischen Zwei-Mandanten-Restore-Drills mit Prüfpunkten P-01 bis P-08; `docs/SP3_02_DECISION_LOG.md` führt zehn offene Entscheidungen D-01 bis D-10 ohne erfundene Werte oder Owner. **Für Backup/Restore existiert repositoryweit kein `measured`-Nachweis**; es wurde kein Restore ausgeführt, kein Backup gelesen und kein Schlüssel erzeugt, rotiert oder angezeigt | lokaler synthetischer Phase-B-Drill kann mit logischem Testdump und Test-only K-01 beginnen; null von zehn Owner-Entscheidungen sind `decided`, daher bleiben Produktionsübertragbarkeit und Freigabe gesperrt; Phase C benötigt D-05/D-06/D-07, Phase D D-08/D-09/D-10; ausgeführter Restore-Drill, Rotationsnachweis und protokolliertes Incident-Tabletop; RPO/RTO bleiben unbestimmt bis Messung **und** Freigabe |
