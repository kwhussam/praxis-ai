...
| SP3-02 | Phase A `audit_completed`; Arbeitspaket insgesamt `blocked_by_owner_decisions` | isolierter Branch `codex/sp3-02-recovery-tabletop`; **Phase A vollständig durchgeführt**: `docs/SP3_02_RECOVERY_INVENTORY.md` inventarisiert am tatsächlichen Code elf Bestandsgruppen (Supabase/Postgres mit 34 RLS-erzwungenen Tabellen, Auth, nicht genutztes Storage, Migrationen/RLS/Grants/54 RPCs, Worker und sechs Secretklassen, Report-/Manifestverschlüsselung, sechs lokale Geräteschlüssel-/Persistenzpfade, Release-Signing, CI-/SBOM-/Attestationsartefakte, neun externe Provider) mit Datenklasse D0–D3, Plattform, Backupautorität, Schlüsselabhängigkeit, Wiederherstellungsstufe R0–R8, Retention-/Löschbezug und Nachweisstatus `measured`/`configured`/`documented`/`unknown`; daraus 24 priorisierte Lücken (7×P1, 8×P2, 6×P3, 3×P4) und der Entwurf des synthetischen Zwei-Mandanten-Restore-Drills mit Prüfpunkten P-01 bis P-08; `docs/SP3_02_DECISION_LOG.md` führt zehn offene Entscheidungen D-01 bis D-10 ohne erfundene Werte oder Owner. **Für Backup/Restore existiert repositoryweit kein `measured`-Nachweis**; es wurde kein Restore ausgeführt, kein Backup gelesen und kein Schlüssel erzeugt, rotiert oder angezeigt | null von zehn Owner-Entscheidungen sind `decided`; Phase B benötigt mindestens D-02 (Backup-/Restoreverfahren) und D-05 (Verwahrung von `DATA_ENCRYPTION_KEY`); Phase C benötigt zusätzlich D-06/D-07, Phase D zusätzlich D-08/D-09/D-10; ausgeführter Restore-Drill mit gemessenen Zeitspannen, Rotationsnachweis und protokolliertes Incident-Tabletop; RPO/RTO bleiben unbestimmt bis Messung **und** Freigabe |
...
- **`DATA_ENCRYPTION_KEY` ist der kritischste Einzelpunkt des Produkts.** `workers/hono/src/index.ts:2416-2451` belegt einen einzigen globalen AES-256-GCM-Schlüssel für alle Mandanten, ohne Keyversion im Envelope, ohne AAD, ohne Wrapping und ohne zweite Version. Er entscheidet über die Lesbarkeit von `security_checks.encrypted_payload`, `reports.encrypted_content`, `assessment_manifests.encrypted_snapshot` und `monitoring_snapshots.encrypted_checks`. Sein Verlust ist der unwiederbringliche Verlust aller Berichte aller Mandanten; seine Kompromittierung trifft alle Mandanten gleichzeitig. Eine Rotation ist am heutigen Stand **technisch nicht durchführbar**, weil ohne Keyversion kein Mischzustand lesbar wäre. Verwahrung, Escrow und Wiederbeschaffungspfad sind repositoryweit `unknown`. Dies ist der Iststand zu dem in ADR-001 Abschnitt 6.1 beschriebenen, nicht implementierten Zielzustand.
- **Die längste Wiederherstellungszeit liegt nicht in der Datenbank.** Ein Restore in ein neues Supabase-Projekt ändert `PRODUCTION_SUPABASE_URL` (Environment-Secret beider Release-Workflows). Ausgelieferte Binaries sind auf die alte URL gepinnt, weshalb ein solcher Restore einen Store-Release mit Reviewdauer erzwingt. Die RPO/RTO-Entscheidung braucht deshalb **zwei** getrennte RTO-Werte. Parallel ist der Release-Signing-Recoverypfad selbst unbelegt – insbesondere ist nicht dokumentiert, ob Google Play App Signing aktiv ist, wovon abhängt, ob ein Keyverlust überhaupt behebbar ist.
...
Positiv belegt: 34 Anwendungstabellen mit `enable` **und** `force row level security`, 242 pgTAP-Assertions, append-only Consent-Registry mit Vorgängerkette und entzogenen `update`/`delete`/`truncate`-Rechten, transaktionale Manifest-/Reporterzeugung mit Hashkette, fail-closed Providerstatus (`not_configured` statt „kein Risiko") sowie der vollständige, bewusste Ausschluss mobiler Daten aus Gerätebackups (`WHEN_UNLOCKED_THIS_DEVICE_ONLY`, `allowBackup="false"`, ausgeschlossene `cloud-backup`- und `device-transfer`-Domänen). Der lokale Kryptovertrag (`lib/inventory/localInventoryCrypto.ts`) ist mit echter AAD und Versionsfeldern strenger als der Cloudpfad.
