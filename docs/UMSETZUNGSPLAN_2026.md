...
| SP2-04 | verification | Unveränderliches, versioniertes und AES-256-GCM-verschlüsseltes Assessment-Manifest; explizit definierte kanonische JSON-/Zahlenregeln; transaktionale/idempotente Manifest+Report-RPC; tenantgebundener serverseitiger Mehrseiten-PDF-Pfad mit Integritätsprüfung und byte-identischer Reproduktion; Decrypt-Fehler liefern fail-closed 409; lokaler HTML-Renderer entfernt; App öffnet Serverbytes über den nativen Teilen-/Öffnen-Dialog, hält sie nur tenantgetrennt in `cacheDirectory` und löscht die Klartextdatei nach Dialogende/Fehler sowie bei Logout/Praxiswechsel; Reporthistorie wird nach Neustart geladen; Altberichte ohne Manifest bleiben lesbar und scheitern beim kanonischen Export geschlossen; Datenschutzexport und Lösch-RPC erweitert; technisches Review ohne P1/P2 bestanden; 19 neue pgTAP-Beweise für RLS, Grants, Cross-Tenant-FK, Atomicity und Idempotenz; Gitleaks und pgTAP im ersten GitHub-Lauf grün; echter native iOS-PDF-Dialog mit Server-PDF und Vorschau nachgewiesen | grüner CI-Gesamtlauf nach Push der Portabilitätsfixes; vollständiger PDF-Export-/Öffnen-/Cleanup-Smoke auf iOS und Android (Android ohne lokales AVD/Gerät); Datenschutz-Sign-off/ADR-001 sowie externe D1-Rechtsprüfung vor `released` |

Verifikation dieses Arbeitsstands: `npm run verify` ist grün (Lint, TypeScript, **412 bestandene Tests**, 6 bewusst übersprungen, 2 bestandene Semantik-Snapshots). Die erwarteten Warn-/Fehlerlogs stammen aus simulierten Provider- und Auditfehlerfällen der Tests. `supabase db test --local` bestand vollständig mit **225 pgTAP-Tests**; darin prüft die neue SP2-04-Suite 19 RLS-, Grant-, `search_path`-, Cross-Tenant-FK-, Atomicity- und Idempotenz-Eigenschaften ohne optionale Jest-Umgebungsvariablen. Der GitHub-Job `rls-pgtap` und Gitleaks liefen nach dem ersten Push grün; die Quality-Stage fand zwei CI-Portabilitätsprobleme (lokale Zeitzone und bewusst optionale Remote-RLS-Konfiguration), deren Korrekturen nun enthalten sind. Der frühere frische Datenbank-Reset wandte alle Migrationen einschließlich SP2-04 an, und der Datenbank-Linter meldete keine neue SP2-04-Warnung. Zusätzlich wurden die iOS- und Android-Metro-Exporte mit den neuen Crypto-/SQLite-Modulen erfolgreich erzeugt. Der SP2-01A-Maestro-Flow bestand im nativen iOS-18.6-Simulator; SQLite und Keychain-Metadaten wurden danach unabhängig von der App auf Mandantentrennung und Klartextfreiheit geprüft. Der eigenständige SP2-03-Flow `14-dashboard-responsive` bestand auf iOS 18.6 im iPhone-SE-Viewport sowohl bei Standardschrift als auch mit `accessibility-extra-large`; dabei gefundene Überläufe in Ergebniszeilen, Risikoüberschrift, Tab-Beschriftungen, Umschalter und Technikmetriken wurden behoben. Für SP2-04 wurde das echte kanonische Server-PDF samt Vorschau im nativen iOS-Dialog nachgewiesen; der vollständige automatisierte Cleanup-Smoke bleibt wegen einer lokal festgefahrenen Docker-Instanz sowie fehlendem Android-Gerät offen.
